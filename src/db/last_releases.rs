...
pub fn last_releases() -> Result<(Vec<ReleaseCard>, i16), Box<dyn Error>> {
	...
	let mut count_stmt =
		connection.prepare("SELECT count(*) FROM releases ORDER BY published DESC LIMIT 30")?;
	...
	let count = count_stmt
		.query_map([], |row| Ok(row.get::<usize, i16>(0)?))?
		.filter_map(|row| row.ok())
		.last()
		.ok_or("No count")?;
...
pub fn last_release(connection: &Connection) -> Result<ReleaseCard, Box<dyn Error>> {
	let mut releases_stmt = connection
		.prepare("SELECT id, title, coverSrc, published FROM releases ORDER BY published DESC LIMIT 1")?;
